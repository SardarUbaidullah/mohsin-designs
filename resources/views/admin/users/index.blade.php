...
@section('content')
        ...
        <div class="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 xl:px-8">
            ...
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-6 lg:mb-8">
                <div class="mb-4 lg:mb-0">
                    ...
                    <p class="text-gray-600 mt-1 sm:mt-2 text-xs sm:text-sm lg:text-base truncate">Manage all system users
            ...
            <div class="bg-white rounded-xl lg:rounded-2xl shadow-sm border border-gray-200 p-4 sm:p-5 lg:p-6 mb-6 lg:mb-8">
                <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3 sm:gap-4">
                    <div class="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                        <!-- Department Filter -->
                        <div class="min-w-0">
                            <label class="block text-xs sm:text-sm font-medium text-gray-700 mb-1 sm:mb-2 truncate">Filter
                                by Department</label>
                            <select id="department_filter"
                                class="w-full px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-200 truncate">
                                <option value="all">All Departments</option>
                                <option value="Not Assigned"
                                    {{ request('department') == 'Not Assigned' ? 'selected' : '' }}>Not Assigned</option>
                        ...
                        <div class="min-w-0">
                            ...
                            <select id="role_filter"
                                ...
                                <option value="user" {{ request('role') == 'user' ? 'selected' : '' }}>Team Member
                                </option>
                    ...
                    <div class="lg:pt-6 mt-2 sm:mt-0">
                        <button id="reset_filters"
                            class="w-full lg:w-auto px-4 sm:px-6 py-2 sm:py-3 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition duration-200 font-medium flex items-center justify-center text-sm sm:text-base">
                            <svg class="w-3 h-3 sm:w-4 sm:h-4 mr-1 sm:mr-2 flex-shrink-0" fill="none"
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-6 gap-4">
                ...
                <div class="lg:hidden">
                    <div class="flex space-x-1 bg-white p-1 rounded-lg shadow-sm border border-gray-200 overflow-x-auto">
                        <button data-role="super_admin"
                            class="filter-tab active flex-1 px-2 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm font-medium rounded-lg bg-purple-100 text-purple-800 whitespace-nowrap transition-all duration-200 min-w-0 truncate">
                            <span class="truncate">Super Admins ({{ $users->where('role', 'super_admin')->count() }})</span>
                        </button>
                        <button data-role="admin"
                            class="filter-tab flex-1 px-2 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm font-medium rounded-lg text-gray-600 hover:bg-gray-100 whitespace-nowrap transition-all duration-200 min-w-0 truncate">
            ...
            <div id="gridView" class="user-view active">
                <div
                    class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-4 gap-3 sm:gap-4 lg:gap-6 mb-6 lg:mb-8">
                    <!-- Super Admins Column -->
                    <div class="user-column active bg-gray-50 rounded-xl lg:rounded-2xl"
                        data-role="super_admin">
                        <div class="flex items-center justify-between mb-3 sm:mb-4 lg:mb-6">
                            <h3
                                class="text-sm sm:text-base lg:text-lg font-semibold text-gray-900 flex items-center min-w-0">
                                <div
                                    class="w-2 h-2 sm:w-2 sm:h-2 lg:w-3 lg:h-3 bg-purple-500 rounded-full mr-2 sm:mr-3 flex-shrink-0">
                                </div>
                                <span class="truncate">Super Admins</span>
                            </h3>
                            <span
                                class="bg-purple-100 text-purple-800 px-2 sm:px-3 py-1 rounded-full text-xs font-medium flex-shrink-0 ml-2">
                        ...
                        <div class="space-y-2 sm:space-y-3 lg:space-y-4">
                            @foreach ($users->where('role', 'super_admin') as $user)
                                <div
                                    class="bg-white rounded-lg lg:rounded-xl shadow-sm border border-gray-200 p-3 sm:p-4 lg:p-5 hover:shadow-md transition-all duration-200">
                                    <div class="flex items-center justify-between mb-2 sm:mb-3 lg:mb-4">
                                        <div class="flex items-center space-x-2 sm:space-x-3 min-w-0 flex-1">
                                            <div
                                                class="w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 rounded-lg lg:rounded-xl bg-purple-500 flex items-center justify-center text-white font-semibold text-xs sm:text-sm lg:text-lg shadow-sm flex-shrink-0">
                                                {{ strtoupper(substr($user->name, 0, 1)) }}
                                            </div>
                                            <div class="min-w-0 flex-1">
                                                <h4
                                                    class="font-semibold text-gray-900 text-xs sm:text-sm lg:text-base truncate">
                                                    {{ $user->name }}</h4>
                                    ...
                                    <div class="flex items-center justify-between text-xs sm:text-sm mb-2 sm:mb-3 lg:mb-4">
                                        <span
                                            class="bg-purple-100 text-purple-800 px-2 sm:px-3 py-1 rounded-lg text-xs font-medium truncate">
                                            Super Admin
                                        </span>
                                    ...
                                    <div class="mb-2 sm:mb-3 lg:mb-4">
                                        <div class="text-xs text-gray-500 flex items-center min-w-0">
                                            <svg class="w-3 h-3 sm:w-3 sm:h-3 lg:w-4 lg:h-4 mr-1 sm:mr-2 flex-shrink-0"
                                    ...
                                    <!-- User Status Section -->
                                    <div
                                        class="mb-2 sm:mb-3 lg:mb-4 p-2 sm:p-3 bg-gray-50 rounded-lg border border-gray-200">
                                        <div class="flex items-center justify-between">
                                            <div class="flex items-center space-x-1 sm:space-x-2 min-w-0">
                                                <svg class="w-3 h-3 sm:w-4 sm:h-4 {{ $user->status === 'active' ? 'text-green-600' : 'text-red-600' }} flex-shrink-0"
                                                    fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                ...
                                                <span class="text-xs font-medium text-gray-700 truncate">Account
                                                    Status</span>
                                            </div>
                                        ...
                                        <p class="text-xs text-gray-500 mt-1 sm:mt-2 truncate">
                                            @if ($user->status === 'active')
                                                ...
                                                @if ($user->created_at)
                                                    <span class="block text-gray-400">Active since:
                                                        {{ $user->created_at->format('M d, Y g:i A') }}</span>
                                                @endif
                                            @else
                                                <span class="text-red-600 font-medium">✗ Inactive - Cannot login</span>
                                                @if ($user->deactivated_at)
                                                    <span class="block text-gray-400">Deactivated:
                                                        {{ $user->deactivated_at->format('M d, Y g:i A') }}</span>
                                                @endif
                                            @endif
                                            @if ($user->id === auth()->id())
                                    ...
                                    <div
                                        class="flex items-center justify-between pt-2 sm:pt-3 lg:pt-4 border-t border-gray-100">
                                        <div
                                            class="text-xs text-gray-500 truncate max-w-[100px] sm:max-w-[120px] lg:max-w-none min-w-0">
                                            @if ($user->client)
                                                <span class="flex items-center min-w-0">
                                                    <svg class="w-3 h-3 sm:w-3 sm:h-3 lg:w-4 lg:h-4 mr-1 flex-shrink-0"
                                                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                                            stroke-width="2"
                                                            d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
                                        ...
                                        <div class="flex space-x-1 sm:space-x-2 flex-shrink-0 ml-2">
                                            <a href="{{ route('users.show', $user) }}"
                                                ...
                                                title="View User">
                                                <svg class="w-3 h-3 sm:w-3 sm:h-3 lg:w-4 lg:h-4" fill="none"
                                                    stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <!-- Managers Column -->
                    <div class="user-column bg-gray-50 rounded-xl lg:rounded-2xl" data-role="admin">
                        <div class="flex items-center justify-between mb-3 sm:mb-4 lg:mb-6">
                            <h3
                                class="text-sm sm:text-base lg:text-lg font-semibold text-gray-900 flex items-center min-w-0">
                                <div
                                    class="w-2 h-2 sm:w-2 sm:h-2 lg:w-3 lg:h-3 bg-blue-500 rounded-full mr-2 sm:mr-3 flex-shrink-0">
                                </div>
                                <span class="truncate">Managers</span>
                            </h3>
                            <span
                                class="bg-blue-100 text-blue-800 px-2 sm:px-3 py-1 rounded-full text-xs font-medium flex-shrink-0 ml-2">
                            ...
                            @foreach ($users->where('role', 'admin') as $user)
                                <div
                                    ...
                                    <div class="flex items-center justify-between mb-2 sm:mb-3 lg:mb-4">
                                        <div class="flex items-center space-x-2 sm:space-x-3 min-w-0 flex-1">
                                            <div
                                                class="w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 rounded-lg lg:rounded-xl bg-blue-500 flex items-center justify-center text-white font-semibold text-xs sm:text-sm lg:text-lg shadow-sm flex-shrink-0">
                                                {{ strtoupper(substr($user->name, 0, 1)) }}
                                            </div>
                                            <div class="min-w-0 flex-1">
                                                <h4
                                    ...
                                    <div class="flex items-center justify-between text-xs sm:text-sm mb-2 sm:mb-3 lg:mb-4">
                                        <span
                                            class="bg-blue-100 text-blue-800 px-2 sm:px-3 py-1 rounded-lg text-xs font-medium truncate">
                                            Manager
                                        </span>
                                        <span class="text-gray-400 text-xs flex-shrink-0 ml-2">
                                            #{{ $user->id }}
                                    ...
                                    <div
                                        ...
                                        <div class="flex items-center justify-between">
                                            <div class="flex items-center space-x-1 sm:space-x-2 min-w-0">
                                                <svg class="w-3 h-3 sm:w-4 sm:h-4 text-gray-600 flex-shrink-0"
                                                    fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                        d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                                </svg>
                                                <span class="text-xs font-medium text-gray-700 truncate">Project
                                                    Creation</span>
                                            </div>
                                            <form action="{{ route('admin.users.toggle-project-permission', $user) }}"
                    ...
                    <!-- Team Members Column -->
                    <div class="user-column bg-gray-50 rounded-xl lg:rounded-2xl" data-role="user">
                        <div class="flex items-center justify-between mb-3 sm:mb-4 lg:mb-6">
                            <h3
                                class="text-sm sm:text-base lg:text-lg font-semibold text-gray-900 flex items-center min-w-0">
                                <div
                                    class="w-2 h-2 sm:w-2 sm:h-2 lg:w-3 lg:h-3 bg-green-500 rounded-full mr-2 sm:mr-3 flex-shrink-0">
                                </div>
                                <span class="truncate">Team Members</span>
                            </h3>
                            <span
                                class="bg-green-100 text-green-800 px-2 sm:px-3 py-1 rounded-full text-xs font-medium flex-shrink-0 ml-2">
                            ...
                            @foreach ($users->where('role', 'user') as $user)
                                <div
                                    ...
                                    <div class="flex items-center justify-between mb-2 sm:mb-3 lg:mb-4">
                                        <div class="flex items-center space-x-2 sm:space-x-3 min-w-0 flex-1">
                                            <div
                                                class="w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 rounded-lg lg:rounded-xl bg-green-500 flex items-center justify-center text-white font-semibold text-xs sm:text-sm lg:text-lg shadow-sm flex-shrink-0">
                                                {{ strtoupper(substr($user->name, 0, 1)) }}
                                            </div>
                                            <div class="min-w-0 flex-1">
                                                <h4
                                    ...
                                    <div class="flex items-center justify-between text-xs sm:text-sm mb-2 sm:mb-3 lg:mb-4">
                                        <span
                                            class="bg-green-100 text-green-800 px-2 sm:px-3 py-1 rounded-lg text-xs font-medium truncate">
                                            Team Member
                                        </span>
                                        <span class="text-gray-400 text-xs flex-shrink-0 ml-2">
                                            #{{ $user->id }}
                    ...
                    <!-- Clients Column -->
                    <div class="user-column bg-gray-50 rounded-xl lg:rounded-2xl" data-role="client">
                        <div class="flex items-center justify-between mb-3 sm:mb-4 lg:mb-6">
                            <h3
                                class="text-sm sm:text-base lg:text-lg font-semibold text-gray-900 flex items-center min-w-0">
                                <div
                                    class="w-2 h-2 sm:w-2 sm:h-2 lg:w-3 lg:h-3 bg-orange-500 rounded-full mr-2 sm:mr-3 flex-shrink-0">
                                </div>
                                <span class="truncate">Clients</span>
                            </h3>
                            <span
                                class="bg-orange-100 text-orange-800 px-2 sm:px-3 py-1 rounded-full text-xs font-medium flex-shrink-0 ml-2">
                            ...
                            @foreach ($users->where('role', 'client') as $user)
                                <div
                                    ...
                                    <div class="flex items-center justify-between mb-2 sm:mb-3 lg:mb-4">
                                        <div class="flex items-center space-x-2 sm:space-x-3 min-w-0 flex-1">
                                            <div
                                                class="w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 rounded-lg lg:rounded-xl bg-orange-500 flex items-center justify-center text-white font-semibold text-xs sm:text-sm lg:text-lg shadow-sm flex-shrink-0">
                                                {{ strtoupper(substr($user->name, 0, 1)) }}
                                            </div>
                                            <div class="min-w-0 flex-1">
                                                <h4
                                    ...
                                    <div class="flex items-center justify-between text-xs sm:text-sm mb-2 sm:mb-3 lg:mb-4">
                                        <span
                                            class="bg-orange-100 text-orange-800 px-2 sm:px-3 py-1 rounded-lg text-xs font-medium truncate">
                                            Client
                                        </span>
                                        <span class="text-gray-400 text-xs flex-shrink-0 ml-2">
                                            #{{ $user->id }}
            ...
            <!-- List View -->
            <div id="listView" class="user-view hidden">
                <div class="bg-white rounded-xl lg:rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                                ...
                                @foreach ($users as $user)
                                    <tr class="hover:bg-gray-50 transition-colors duration-150">
                                        <td class="px-3 sm:px-4 lg:px-6 py-4 whitespace-nowrap">
                                            ...
                                            @if ($user->role === 'super_admin') bg-purple-500
                                            @elseif($user->role === 'admin') bg-blue-500
                                            @elseif($user->role === 'user') bg-green-500
                                        ...
                                        <td class="px-3 sm:px-4 lg:px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                        ...
                                        <td class="px-3 sm:px-4 lg:px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center">
                                                ...
                                                <span
                                                    class="ml-2 text-xs font-medium {{ $user->status === 'active' ? 'text-green-600' : 'text-red-600' }}">
                                                    {{ $user->status === 'active' ? 'Active' : 'Inactive' }}
                                                    @if ($user->id === auth()->id())
                                                        <span class="block text-orange-600">(Your account)</span>
                                                    @endif
                                                </span>
                                            </div>
                                        </td>
                                        <td class="px-3 sm:px-4 lg:px-6 py-4 whitespace-nowrap">
                                            @if ($user->role === 'admin')
                                                <form
                                                    action="{{ route('admin.users.toggle-project-permission', $user) }}"
                                                ...
                                                <span
                                                    class="ml-2 text-xs font-medium {{ $user->can_create_project ? 'text-green-600' : 'text-red-600' }}">
                                                    {{ $user->can_create_project ? 'Can Create' : 'Cannot Create' }}
                                                </span>
                                            @else
            ...
            @if ($users->count() == 0)
                <div
                    class="bg-white rounded-xl lg:rounded-2xl shadow-sm border border-gray-200 text-center py-8 sm:py-12 lg:py-16 px-4">
                    <div
                        class="w-12 h-12 sm:w-16 sm:h-16 lg:w-20 lg:h-20 bg-gray-100 rounded-xl lg:rounded-2xl flex items-center justify-center mx-auto mb-3 sm:mb-4 lg:mb-6 shadow-sm">
                    ...
                    <h3 class="text-base sm:text-lg lg:text-xl font-semibold text-gray-900 mb-1 sm:mb-2 lg:mb-3 truncate">
                        No users found</h3>
                    <p class="text-gray-600 mb-4 sm:mb-6 lg:mb-8 max-w-md mx-auto text-xs sm:text-sm lg:text-base">Get
                        started by adding your first team member or client to the system</p>
                    <a href="{{ route('users.create') }}"
                        class="bg-blue-600 hover:bg-blue-700 text-white px-4 sm:px-6 lg:px-8 py-2 sm:py-3 rounded-lg lg:rounded-xl font-medium transition duration-200 inline-flex items-center justify-center shadow-sm w-full sm:w-auto text-sm sm:text-base">
    ...
    <script>
        // Professional Filters and View Toggle
                ...
                filterTabs.forEach(tab => {
                    tab.addEventListener('click', function() {
                        ...
                        filterTabs.forEach(t => {
                            t.classList.remove('active', 'bg-purple-100',
                            'text-purple-800');
