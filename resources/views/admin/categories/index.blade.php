    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="mb-8">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <div class="mb-4 sm:mb-0 max-w-full">
                    <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 truncate">Categories Management</h1>
                    <p class="text-gray-600 mt-2 text-sm sm:text-base truncate max-w-full">Organize and manage your content categories efficiently</p>
                </div>
                <div class="flex items-center space-x-2 sm:space-x-4">
                    <!-- View Toggle -->
                    <div class="flex items-center bg-white rounded-xl shadow-sm border border-gray-200 p-1">
                        <button id="gridViewBtn" class="p-2 rounded-lg text-blue-600 bg-blue-100 transition-all duration-200 view-toggle" data-view="grid">
                            <svg class="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"/>
                            </svg>
                        </button>
                        <button id="listViewBtn" class="p-2 rounded-lg text-gray-700 hover:bg-gray-100 transition-all duration-200 view-toggle" data-view="list">
                            <svg class="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div id="gridView" class="categories-view">
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6">
                ...
                <div class="category-card bg-white rounded-2xl shadow-lg border border-gray-200/60 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 overflow-hidden">
                    <!-- Color Header -->
                    ...
                    <div class="p-4 sm:p-6">
                        ...
                        <div class="flex items-start justify-between mb-4">
                            <div class="flex items-center space-x-2 sm:space-x-3 min-w-0">
                                <div class="w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center text-white font-semibold shadow-sm flex-shrink-0" style="background-color: {{ $category->color }}">
                                    {{ strtoupper(substr($category->name, 0, 1)) }}
                                </div>
                                <div class="min-w-0 flex-1">
                                    <h3 class="text-base sm:text-lg font-semibold text-gray-900 truncate" title="{{ $category->name }}">{{ $category->name }}</h3>
                        ...
                        <div class="mb-4 w-full">
                            <p class="text-xs sm:text-sm text-gray-600 truncate" title="{{ $category->description }}">
                        ...
                        <div class="flex items-center justify-between text-xs sm:text-sm text-gray-500 mb-4">
                            ...
                            <div class="flex items-center space-x-1 flex-shrink-0 ml-2">
                                ...
                                <span class="text-xs font-mono truncate max-w-[60px] sm:max-w-none" title="{{ $category->color }}">{{ $category->color }}</span>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-between pt-4 border-t border-gray-100">
                            <div class="flex space-x-1 sm:space-x-2 min-w-0">
                                ...
                                <button type="button"
                                    ...
                                    <span class="truncate">{{ $category->is_active ? 'Deactivate' : 'Activate' }}</span>
                                </button>
                            </div>

                            <form action="{{ route('categories.destroy', $category) }}" method="POST" class="inline flex-shrink-0">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                ...
                <div class="col-span-full">
                    <div class="bg-white rounded-2xl shadow-lg border border-gray-200/60 text-center py-8 sm:py-12 px-4 sm:px-6">
                        <div class="w-12 h-12 sm:w-16 sm:h-16 bg-gray-100 rounded-2xl flex items-center justify-center mx-auto mb-3 sm:mb-4 shadow-sm">
                            <svg class="w-6 h-6 sm:w-8 sm:h-8 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                            </svg>
                        </div>
                        <h3 class="text-base sm:text-lg font-semibold text-gray-900 mb-2 truncate">No categories found</h3>
                        <p class="text-gray-600 mb-4 sm:mb-6 max-w-md mx-auto text-sm sm:text-base truncate">Get started by creating your first category to organize your content</p>
                        <a href="{{ route('categories.create') }}"
                ...
                <div class="overflow-x-auto">
                    <table class="w-full min-w-[600px]">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider truncate">Category</th>
                                <th class="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider truncate">Description</th>
                                <th class="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider truncate">Status</th>
                                <th class="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider truncate">Color</th>
                                <th class="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider truncate">Created</th>
                                <th class="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider truncate">Actions</th>
                            </tr>
                        </thead>
                            ...
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                <td class="px-4 sm:px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center min-w-0">
                                        <div class="w-8 h-8 sm:w-10 sm:h-10 rounded-lg flex items-center justify-center text-white font-semibold shadow-sm mr-2 sm:mr-3 flex-shrink-0" style="background-color: {{ $category->color }}">
                                            {{ strtoupper(substr($category->name, 0, 1)) }}
                                        </div>
                                        <div class="min-w-0 flex-1">
                                            <div class="text-sm font-medium text-gray-900 truncate max-w-[120px] sm:max-w-xs" title="{{ $category->name }}">{{ $category->name }}</div>
                                            <div class="text-xs text-gray-500 truncate">ID: {{ $category->id }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-4 sm:px-6 py-4">
                                    <div class="min-w-0 max-w-[200px]">
                                        @if($category->description)
                                        <div class="text-xs sm:text-sm text-gray-600 truncate" title="{{ $category->description }}">
                                            {{ $category->description }}
                                        </div>
                                        @else
                                        <div class="text-xs sm:text-sm text-gray-400 italic">No description</div>
                                        @endif
                                    </div>
                                </td>
                                <td class="px-4 sm:px-6 py-4 whitespace-nowrap">
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium {{ $category->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }} truncate">
                                        {{ $category->is_active ? 'Active' : 'Inactive' }}
                                    </span>
                                </td>
                                <td class="px-4 sm:px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="w-3 h-3 rounded-full mr-2 flex-shrink-0" style="background-color: {{ $category->color }}"></div>
                                        <span class="text-xs sm:text-sm text-gray-900 font-mono truncate max-w-[60px] sm:max-w-none" title="{{ $category->color }}">{{ $category->color }}</span>
                                    </div>
                                </td>
                                <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-xs sm:text-sm text-gray-500 truncate">
                            ...
                            <tr>
                                <td colspan="6" class="px-4 sm:px-6 py-8 sm:py-12 text-center">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <h3 class="text-base sm:text-lg font-semibold text-gray-900 mb-2 truncate">No categories found</h3>
                                        <p class="text-gray-600 mb-4 sm:mb-6 max-w-md text-xs sm:text-sm truncate">Get started by creating your first category to organize your content</p>
                                        <a href="{{ route('categories.create') }}"
                                           class="inline-flex items-center px-4 sm:px-6 py-2 sm:py-3 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white font-medium rounded-xl shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-200 text-sm sm:text-base">
                                            <svg class="w-4 h-4 sm:w-5 sm:h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/>
        ...
        <div class="mt-6 sm:mt-8 grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
            ...
            <div class="bg-white rounded-2xl shadow-lg border border-gray-200/60 p-4 sm:p-6">
                <div class="flex items-center">
                    <div class="w-10 h-10 sm:w-12 sm:h-12 bg-green-100 rounded-xl flex items-center justify-center mr-3 sm:mr-4 flex-shrink-0">
                    ...
                    <div class="min-w-0">
                        ...
                        <p class="text-xl sm:text-2xl font-bold text-gray-900 truncate">{{ $categories->where('is_active', true)->count() }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-lg border border-gray-200/60 p-4 sm:p-6">
                <div class="flex items-center">
                    <div class="w-10 h-10 sm:w-12 sm:h-12 bg-purple-100 rounded-xl flex items-center justify-center mr-3 sm:mr-4 flex-shrink-0">
                    ...
                    <div class="min-w-0">
                        ...
                        <p class="text-xl sm:text-2xl font-bold text-gray-900 truncate">{{ $categories->pluck('color')->unique()->count() }}</p>
                    </div>
                </div>
            </div>
        </div>
