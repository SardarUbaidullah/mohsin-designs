@php
    ...
    $tasksIndexRoute = $isSuperAdmin ? route('tasks.index') : route('manager.tasks.index');
    $tasksEditRoute = $isSuperAdmin ? route('tasks.edit', $task->id) : route('manager.tasks.edit', $task->id);
    $projectsShowRoute = $isSuperAdmin ? route('projects.show', ':id') : route('manager.projects.show', ':id');
    $milestonesShowRoute = $isSuperAdmin ? route('milestones.show', ':id') : route('manager.milestones.show', ':id');
    $teamShowRoute = $isSuperAdmin ? route('users.show', ':id') : route('manager.team.show', ':id');
    $subtasksIndexRoute = $isSuperAdmin ? route('subtasks.index', $task->id) : route('manager.subtasks.index', $task->id);
    $subtasksCreateRoute = $isSuperAdmin ? route('subtasks.create', $task->id) : route('manager.subtasks.create', $task->id);

    // Fix subtask edit route - check which route structure is used in your application
    // Option 1: If using single parameter (subtask ID only)
    $subtasksEditRoute = $isSuperAdmin ? route('subtasks.edit', ':id') : route('manager.subtasks.edit', ':id');

    // Option 2: If using nested route (taskId/subtaskId)
    // $subtasksEditRoute = $isSuperAdmin ? route('subtasks.edit', ['task' => $task->id, 'subtask' => ':id']) : route('manager.subtasks.edit', ['task' => $task->id, 'subtask' => ':id']);

    $tasksDestroyRoute = $isSuperAdmin ? route('tasks.destroy', $task->id) : route('manager.tasks.destroy', $task->id);
@endphp
            ...
            <div class="xl:col-span-2 space-y-8">
                                ...
                                <div>
                                    ...
                                    <div class="space-y-4">
                                        ...
                                        <div class="flex justify-between items-center py-3 border-b border-gray-100">
                                            ...
                                            <a href="{{ str_replace(':id', $task->project_id, $projectsShowRoute) }}"
                                               class="text-sm font-semibold text-blue-600 hover:text-blue-800 transition-colors duration-200">
                                                {{ $task->project->name }}
                                                @if($isSuperAdmin && $task->project->manager)
                                                    <span class="text-xs text-gray-500">({{ $task->project->manager->name }})</span>
                                                @endif
                                            </a>
                                        </div>
                                        <div class="flex justify-between items-center py-3 border-b border-gray-100">
                                            <span class="text-sm font-medium text-gray-600">Created By</span>
                                            <span class="text-sm font-semibold text-gray-900">{{ $task->creator->name ?? 'System' }}</span>
                                        </div>
                                        @if($task->milestone)
                                        <div class="flex justify-between items-center py-3 border-b border-gray-100">
                                            <span class="text-sm font-medium text-gray-600">Milestone</span>
                                            <div class="flex items-center">
                                                <a href="{{ str_replace(':id', $task->milestone->id, $milestonesShowRoute) }}"
                                                   class="text-sm font-semibold text-indigo-600 bg-indigo-50 px-2 py-1 rounded-lg hover:text-indigo-800 transition-colors duration-200">
                                                    {{ $task->milestone->title }}
                                                </a>
                                            </div>
                                        </div>
                                ...
                                <div>
                                    ...
                                    <div class="space-y-4">
                                        <div class="flex justify-between items-center py-3 border-b border-gray-100">
                                            ...
                                            <div class="flex items-center">
                                                @if($task->assignee)
                                                <a href="{{ str_replace(':id', $task->assignee->id, $teamShowRoute) }}"
                                                   class="flex items-center text-sm font-semibold text-gray-900 hover:text-blue-600 transition-colors duration-200">
                    ...
                    <div class="p-6">
                        ...
                        <div class="mt-6 pt-6 border-t border-gray-100">
                            ...
                            <div class="bg-indigo-50 rounded-xl p-4 border border-indigo-200">
                                <p class="text-sm text-gray-700 leading-relaxed whitespace-pre-line">{{ $task->milestone->description }}</p>
                            </div>
                        </div>
                        @endif
                    </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <div class="flex items-center space-x-3">
                            <a href="{{ $subtasksIndexRoute }}"
                               class="text-blue-600 hover:text-blue-800 text-sm font-medium">
                                View All
                            </a>
                            <a href="{{ $subtasksCreateRoute }}"
                    ...
                    @if($task->subtasks && $task->subtasks->count() > 0)
                        <div class="space-y-3">
                            ...
                            <div class="flex items-center justify-between p-3 border border-gray-200 rounded-lg hover:bg-gray-50 transition duration-150">
                                ...
                                <div class="flex items-center space-x-2">
                                    <a href="{{ str_replace(':id', $subtask->id, $subtasksEditRoute) }}"
            ...
            <div class="space-y-8">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-red-200 overflow-hidden transition-all duration-300 hover:shadow-md">
                    ...
                    <div class="p-6">
                        ...
                        <form action="{{ $tasksDestroyRoute }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
